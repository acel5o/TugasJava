...
import com.Mahasiswa.util.CustomErrorType;
...
public class RestApiController {
    ...
    public ResponseEntity<?> detailNilaiMhs(@PathVariable("id") Long id) {
        ...
        List<Details> detail = mhsService.findByIdDetail(id);
    ...
    public ResponseEntity<?> createMhs(@PathVariable("id") Long id,@RequestBody Details mhs) {
        logger.info("Creating Mahasiswa : {}", mhs);
    ...
    public ResponseEntity<?> updateMhs(@PathVariable("id") Long id, @RequestBody Details mhs) {
        logger.info("Updating Mahasiswa with id {}", id);

        List<Details> currentDetail = mhsService.findByIdDetail(id);
        if (currentDetail == null) {
            logger.error("Unable to update. Mahasiswa with id {} not found.", id);
            return new ResponseEntity<>(new CustomErrorType("Unable to upate. Mahasiswa with id " + id + " not found."),
                    HttpStatus.NOT_FOUND);
        }
        for (Details obj: currentDetail ){
